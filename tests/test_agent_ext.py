...
async def test_history_preserved_across_prompts(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr(subagent_strategy, "create_subagent_for_model", lambda *_args, **_kwargs: executor)
    monkeypatch.setattr(subagent_strategy, "create_subagent_planner_for_model", lambda *_args, **_kwargs: planner)
    agent = ACPAgent(conn)
    session = await agent.new_session(cwd=str(tmp_path), mcp_servers=[])
