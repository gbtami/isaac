...
async def test_terminal_lifecycle(tmp_path: Path):
    ...
    # wait for exit
    exit_resp = await agent.wait_for_terminal_exit(session_id=session.session_id, terminal_id=term_id)
    assert exit_resp.exit_code == 0

    # create long-running and kill it
    create_resp2 = await agent.create_terminal(command="sleep", session_id=session.session_id, args=["5"])
    term_id2 = create_resp2.terminal_id
    await agent.kill_terminal_command(session_id=session.session_id, terminal_id=term_id2)
    exit_resp2 = await agent.wait_for_terminal_exit(session_id=session.session_id, terminal_id=term_id2)
